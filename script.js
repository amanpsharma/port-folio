...
(function () {
  ...
  const link =
    "color:#6b5dff;font:600 12px/1.5 monospace;text-decoration:underline";
  console.log(
...
(function () {
  ...
  document.addEventListener("keydown", (e) => {
    if (e.key === "Escape" && ham.classList.contains("open")) set(false);
  });
})();
...
(function () {
  if (reduced) {
    document
      .querySelectorAll("[data-stagger], .reveal")
...
(function () {
  ...
  els.forEach((el) => {
    let raf = 0;
    const strength =
      el.tagName === "A" && el.classList.contains("btn-icon-link") ? 0.45 : 0.3;
...
/* -------------- Pause hero animations when offscreen -------------- */
(function () {
...
(function () {
  const fields = document.querySelectorAll(".ff[data-validate]");
  if (!fields.length) return;

  function validate(field) {
    const input = field.querySelector("input, textarea");
    const msg = field.querySelector(".ff-msg");
    if (!input) return;
    const v = input.value.trim();
    if (!v) {
      field.classList.remove("ff--ok", "ff--error");
    ...
    if (kind === "email") {
      ...
      ok = v.length >= 2;
      err = "A name needs at least 2 characters";
    } else if (kind === "message") {
      ok = v.length >= 10;
      err = "A few more words please (min 10 chars)";
    } else {
      ok = input.checkValidity();
      err = "Please fix this field";
    }
    field.classList.toggle("ff--ok", ok);
    field.classList.toggle("ff--error", !ok);
    if (msg) msg.textContent = ok ? "Looks good" : err;
  }
  ...
  fields.forEach((field) => {
    ...
    input.addEventListener("input", () => {
      if (
        field.classList.contains("ff--error") ||
        field.classList.contains("ff--ok")
      ) {
        validate(field);
      }
    });
...
(function () {
  ...
  function reply(q) {
    ...
    if (/(under armour|a\/?b|ab test|optimi[sz]e|experiment)/.test(s))
      return "At Litmus7 he runs A/B testing for Under Armour — designing experiments, shipping React variants, and rolling measured winners into permanent UI.";
    if (
      /(adobe|analytic|tracking|viewport|click.?through|add.?to.?bag)/.test(s)
    )
      return "On the Litmus7 / Under Armour work he implements Adobe Analytics tracking on recommendation zones — viewport visibility, click-through, scroll & hover, and add-to-bag events.";
    if (/(micro.?front|single.?spa|architecture|module)/.test(s))
      return "He's built a Micro Frontend POC with Single-SPA — splitting large apps into independent, scalable modules with isolated deploys.";
    if (/(next|nextjs|next\.js)/.test(s))
      return "Next.js side projects, including an e-commerce product management app with full CRUD + image upload, backed by MongoDB.";
    if (/(mongo|database|crud|node)/.test(s))
      return "Side-project full-stack work: Next.js + MongoDB for an e-commerce product management app with image uploads and structured backend integration.";
    if (/(tailwind)/.test(s))
      return "Yes — Tailwind CSS in production for utility-first, scalable styling on side projects.";
    if (/(movie|ticket|booking)/.test(s))
      return "Movie Ticket Booking — a React side project: responsive booking UI with smooth user flow, seat selection, and tight state management.";
    ...
    if (/(experience|years|how long|senior)/.test(s))
      return "6+ years building production React apps — from ad tech to large-scale e-commerce, plus modern side projects in Next.js, MongoDB, and Single-SPA.";
    if (/(skill|tech|stack|tools|language)/.test(s))
      return "React, Next.js, JavaScript (ES6+), Tailwind, Redux, Material UI. Adobe Analytics + A/B Testing in production. Side-project Node.js + MongoDB. Micro frontends with Single-SPA.";
    if (/(project|work|portfolio|case stud)/.test(s))
      return "Featured: A/B testing for Under Armour (Litmus7). Side projects: Micro Frontend POC (Single-SPA), E-commerce Product Mgmt (Next.js + MongoDB), Movie Ticket Booking (React). Plus the Costrategix ad-serving platform.";
    if (/(focus|exploring|learning|study|currently)/.test(s))
      return "Currently exploring: advanced React + Next.js patterns, micro frontend architecture, performance, and UI/UX craft.";
    ...
    if (/(resume|cv|download)/.test(s))
      return "Drop me an email and I'll send a fresh PDF.";
